...
| F-08 | 참고 | `reports/…/04.test/TEST_REPORT.md` AT-07 | AC-08의 "반환 뒤 `running` 아님" 증거는 `AGENT_ARCHIVE_AFTER_RUN=true`(아카이브로 `closed`)에서만 나왔다. `false`일 때 `interrupt`+10초 settle만으로 정지하는지는 미검증. 다음 iteration에 `false` 변형을 한 번 추가하면 좋다. | AC-08 |
| F-09 | 참고 | `reports/…/04.test/harness/at12-sigterm.ts` | AC-15 후반은 `main.ts`가 아니라 동일 배선 하네스로 측정했다(계획서의 대체 방법). `main.ts:49-55`의 순서는 코드로 확인했으나 DB `destroy()`를 포함한 실제 종료 시간은 미측정이다. | AC-15 |
| F-10 | 참고 | `reports/20260917_0925_paseo_agent_command/03.implementation/.gitkeep` | 워크플로우 표는 Implementation 산출물을 "코드 자체(디렉토리만 생성 X)"로 정의하는데 빈 디렉토리와 `.gitkeep`이 있다. 삭제해도 무방하다. | - |
...
| F-10 | 참고 (이월) | `reports/…/03.implementation/.gitkeep` | 빈 디렉토리·`.gitkeep` 여전히 존재 | - |
...
| F-15 | 참고 | `reports/…/02.architecture/SOFTWARE_ARCHITECTURE.md` §5 | "앱 → Paseo 데몬" 인터페이스 표에 iteration 2에서 추가된 `providers.listModels` 호출이 빠져 있다. 2.6a·§4·FLOW_CHART에는 반영돼 있어 문서 전체로는 일치한다 | AC-03 |
| F-16 | 참고 | `reports/…/04.test/harness/at12-cancel.ts:22` | `agentSnapshot(client, outcome.agentId)`에 `string \| null`이 된 `agentId`를 그대로 넘긴다. 하네스는 `tsconfig` `include`(`src/**`) 밖이라 `npm run typecheck`에 걸리지 않고 런타임에서는 abort 시점상 항상 non-null이라 동작하지만, 하네스를 타입검사하면 실패한다. 프로덕션 코드와 무관 | AC-14 |
